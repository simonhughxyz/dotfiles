...
# c.editor.command = ["foot", "-e", "zsh", "-i", "-c", "'nvim", "-f", "{file}", "-c", "normal{line}G{column0}l", "-c", "set ft=markdown'"]
c.editor.command = ["foot", "-e", "zsh", "-i", "-c", "nvim -f {file} -c normal{line}G{column0}l -c 'set ft=markdown'"]
...
c.fileselect.folder.command = ["foot", "-e", "lf", "-print-selection", "-selection-path", "{}"]
c.fileselect.single_file.command = ["foot", "-e", "lf", "-print-selection", "-selection-path", "{}"]
c.fileselect.multiple_files.command = ["foot", "-e", "lf", "-print-selection", "-selection-path", "{}"]
...
c.url.searchengines = {
    ...
    # SlackBuilds
    "sbo" : "https://slackbuilds.org/result/?search={}"
}
...
# chromium
config.bind(leader + "fc", 'hint links spawn --detach www -c "{hint-url}"')
config.bind(leader + "rc", 'hint -r links spawn --detach www -c "{hint-url}"')
config.bind(leader + "uc", 'spawn --detach www -c "{url}"')
# mpv
config.bind(leader + "fM", 'hint links spawn mpv "{hint-url}"')
config.bind(leader + "rM", 'hint -r links spawn mpv "{hint-url}"')
config.bind(leader + "uM", 'spawn mpv "{url}"')
# ytdl
...
config.bind("coc", "container-open career")
...
# popups
config.bind("cep", "set -p -u {url:host} content.javascript.can_open_tabs_automatically true")
config.bind("cdp", "set -p -u {url:host} content.javascript.can_open_tabs_automatically false")
config.bind("ceup", "set -p -u {url} content.javascript.can_open_tabs_automatically true")
config.bind("cdup", "set -p -u {url} content.javascript.can_open_tabs_automatically false")
...
config.bind("gc", "tab-clone")
config.bind("gs", "spawn --userscript qute-save-article")
...
# load local config
localconfig = str(config.configdir / 'local-config.py')
